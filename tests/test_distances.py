...
def test_get_distance_param_styles(client, case):
    ...
    # sanity bounds to avoid flakiness
    assert 0 <= data["dist_to_park_km"] < 2.0
    assert 0 <= data["dist_to_bus_km"] < 5.0
    assert 0 <= data["dist_to_train_km"] < 5.0
    assert 2.0 <= data["dist_to_city_center_km"] < 12.0
    assert 10.0 <= data["dist_to_airport_km"] < 40.0
    assert 0.2 <= data["dist_to_university_km"] < 12.0
