...
def some_cleaning(df, cat_df):
    ...
    df.loc[:, cat_df.query(
        "category=='PROXIMITY'").feature.unique()
    ] = df.loc[:, cat_df.query("category=='PROXIMITY'").feature.unique()].fillna(9999)
    ...
    df = pd.concat([
        df,
        pd.get_dummies(
            df[['privacy', 'room_type', 'host_response_time']],
            drop_first=True)
    ], axis=1).drop(
        columns=['privacy', 'room_type',
                 'host_response_time', 'amenities', 'price']
    )
    return df
