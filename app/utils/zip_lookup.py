...
def _ensure_zip_ctx(store) -> None:
    ...
    if zips.crs is None or int(gpd.GeoSeries([Point(0, 0)], crs=4326).to_crs(3857).crs.to_epsg() or 3857) != int(zips.crs.to_epsg() or 0):
...
def _zip_for_xy_3857(x: float, y: float, sid: int) -> Optional[str]:
    ...
    mask = (x >= ctx["minx"]) & (x <= ctx["maxx"]) & (
        y >= ctx["miny"]) & (y <= ctx["maxy"])
    if mask.any():
        cand = zips.loc[mask]
    ...
    try:
        ...
        r = 300.0
        rb = (x - r, y - r, x + r, y + r)
        near_mask = (ctx["minx"] <= rb[2]) & (ctx["maxx"] >= rb[0]) & (
            ctx["miny"] <= rb[3]) & (ctx["maxy"] >= rb[1])
