...
class PropertyInvestmentAnalyzer:
    ...
    def _build_analysis_prompt(
        ...
        ml_predictions = []
        if price_pred:
            ml_predictions.append(f"- Predicted Nightly Rate: ${price_pred:.2f}")
        if occ_pred:
            occupancy_rate = (occ_pred / 365) * 100
            ml_predictions.append(f"- Predicted Annual Occupancy: {occ_pred:.0f} days ({occupancy_rate:.1f}%)")
        if rev_pred:
            ml_predictions.append(f"- Predicted Annual Revenue: ${rev_pred:,.2f}")
        
        predictions_text = "\n".join(ml_predictions) if ml_predictions else "ML predictions not available"
        ...
        if property_features:
            ...
            if property_features.get('dist_to_airport_km'):
                feature_lines.append(f"- Distance to Airport: {property_features['dist_to_airport_km']:.1f} km")
            if property_features.get('dist_to_train_km'):
                feature_lines.append(f"- Distance to Train: {property_features['dist_to_train_km']:.1f} km")
            if property_features.get('dist_to_city_center_km'):
                feature_lines.append(f"- Distance to City Center: {property_features['dist_to_city_center_km']:.1f} km")
            
            # Demographics
            if property_features.get('median_income'):
                feature_lines.append(f"- Area Median Income: ${property_features['median_income']:,.0f}")
            if property_features.get('median_home_value'):
                feature_lines.append(f"- Area Median Home Value: ${property_features['median_home_value']:,.0f}")
